        ...
        for user in users:
            result = await session.execute(
                select(MonthlyBudget).where(
                    MonthlyBudget.user_id == user.user_uuid,
                    MonthlyBudget.month_start == month_start
                )
            )
            if result.scalar():
                logger.info(f"⏭ Skipping user {user.user_uuid}, budget already exists for {month_start}")
                continue
            ...
            result = await session.execute(
                select(func.sum(FixedExpense.amount)).where(FixedExpense.user_id == user.user_uuid)
            )
            ...
            result = await session.execute(
                select(MonthlyBudgetAdjustment).where(
                    MonthlyBudgetAdjustment.user_id == user.user_uuid,
                    MonthlyBudgetAdjustment.month == month_start,
                    MonthlyBudgetAdjustment.apply_permanently == True,
                    MonthlyBudgetAdjustment.processed == False
                )
            )
            ...
            result = await session.execute(
                select(SavingsBalance).where(SavingsBalance.user_id == user.user_uuid)
            )
            savings = result.scalar()
            if not savings:
                savings = SavingsBalance(user_id=user.user_uuid, amount=0.0)
                session.add(savings)
            ...
            result = await session.execute(
                select(func.sum(DailyExpense.amount)).where(
                    DailyExpense.user_id == user.user_uuid,
            ...
            if log_parts:
                logger.info(
                    f"🧮 Final savings for user {user.user_uuid}: {savings.amount:.2f} = " + " + ".join(log_parts)
                )
            else:
                logger.info(f"🧮 Final savings for user {user.user_uuid}: {savings.amount:.2f} (No change)")

            logger.info(
                f"📝 Creating budget for user {user.user_uuid} on {month_start}: income={income}, fixed={fixed_total}, "
